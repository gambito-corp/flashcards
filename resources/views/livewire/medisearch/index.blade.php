...
{{--                                wire:click="toggleChatGroup('{{ $group }}')"--}}
        ...
        <div class="">
            ...
            <div class="flex  mt-8 flex-col items-center justify-center h-full w-full bg-[#f3f8fd] ">
                ...
                <div class="w-full max-w-2xl px-8 space-y-8 md:px-0">
                    <div class="space-y-2">
                        <div class="gap-2 text-sm font-semibold text-center text-gray-600">
...
{{--                                            wire:click="$set('question', '{{ $question }}')"--}}
            ...
            @else
                ...
                <div class="flex flex-col h-full bg-[#f3f8fd]">
                    <!-- Encabezado del chat -->
                    <div id="date-content"
                         class="flex flex-wrap items-center justify-between p-4 ml-0 bg-white shadow md:ml-48">
                        ...
                        @if(!Auth::user()->hasAnyRole('root') && Auth::user()->status == 0)
                            <a
                                href="{{ route('planes') }}"
                                class="flex items-center justify-center px-6 py-3 text-lg font-bold text-white transition rounded-full shadow-lg bg-gradient-to-r from-blue-500 to-purple-500 hover:scale-105"
                            >
                                Actualizar a Pro
                    ...
                    <!-- Área de mensajes -->
                    <div class="flex-1 p-4 space-y-3 overflow-y-auto" id="chat-messages">
                        @foreach($messages as $message)

                            @if($message['from'] === 'user')
                                <div class="flex pt-3">
                                    <div class=" text-[17px]   py-2  my-2 w-fit text-[#195b81] font-semibold "><i class="mr-1 fa-regular fa-comment"></i>
                                        {{ $message['text'] }}
                                    </div>
                                </div>
                            @elseif($message['from'] === 'articles')
                                <div class="relative flex flex-wrap gap-4 article-section">
                        ...
                        <div wire:loading.delay wire:target="sendMessage" class="flex justify-start">
                            <div class="flex items-center bg-[#dfe9ef] text-gray-800 px-4 py-2 rounded-lg my-2">
                                <i class="mr-4 text-xl fa-regular fa-hourglass animate-spin-360"></i>
                    ...
                    <div class="p-4 bg-white border-t border-[#d9e6f7]">
                        <div id="chat-content"
                            class="flex flex-wrap items-center px-4 py-4 ml-0 shadow md:px-6 md:ml-48 py-4bg-white rounded-2xl">
                            <input type="text"
                            ...
                            <label
                                class="inline-flex items-center order-1 ml-4 cursor-pointer select-none md:order-none">
...
{{--        </div>--}}
{{--        @if(!Auth::user()->hasAnyRole('root') && Auth::user()->status == 0)--}}
{{--            <div class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 rounded-2xl">--}}
{{--                <a--}}
{{--                    href="{{ route('planes') }}"--}}
{{--                    class="flex items-center justify-center px-6 py-3 text-lg font-bold text-white transition rounded-full shadow-lg bg-gradient-to-r from-blue-500 to-purple-500 hover:scale-105"--}}
{{--                >--}}
