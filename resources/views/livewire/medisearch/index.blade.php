<div class="fixed inset-0 top-[65px] md:top-[75px] left-0 right-0 bottom-0 z-1 bg-[#f3f8fd] flex">
  ...
  dark:[&::-webkit-scrollbar-thumb]:[#d1d5dc]">
            ...
            <div class="flex flex-col flex-1 h-full container-bot transition-all duration-300 w-full ">
                ...
                @if($messages->isEmpty())
                    <div
                        class="transition-all duration-300 ml-auto mt-8 flex flex-col items-center justify-center h-full bg-[#f3f8fd] w-full {{ $openSidebar ? 'sm:w-4/5' : 'sm:w-full' }}">
                        <!-- Logo y título -->
                            ...
                            @foreach($messages as $message)
                                ...
                                @elseif($message['from'] === 'bot')
                                    <div id="answer-content"
                                         class="md:flex block justify-start ml-0 md:ml-20  md:mr-10 mr-0 overflow-hidden">
                                        <div class="justify-start md:flex block">
                                            <div class="bg-white p-4 rounded-2xl  border border-[#d9e6f7] text-left">
                                                ...
                                                @else
                                                    <div id="answer-div" class="text-[#444] answer-div text-[15px]">
                                                        {!! $content !!}
                                                    </div>
                                                @endif

                                                @if(!empty($message['references']))
                                                    <div class="flex flex-wrap gap-2 mt-2 hidden">
    ...
    <div class="ml-0 md:ml-20"
         x-data="{ showArticles: {{ isset($message['is_new']) && $message['is_new'] !== 'false' ? 'false' : 'true' }} }"
         x-show="showArticles"
         x-transition
         @typing-finished.window="showArticles = true"
         id="articles-{{ $loop->index }}">
        <div class="text-left">
 ...
 @if(count($message['data']) >= 3)
    ...
    <button
    type="button"
    onclick="toggleArticles(event, this)"
    ...
    <script>
function toggleArticles(event, button) {
    event.preventDefault();
    event.stopPropagation();

    const container = button.closest('.extra-article-toggle').parentElement;
    const extraArticles = container.querySelectorAll('.extra-article');
    const isHidden = extraArticles[0]?.classList.contains('hidden');

    if (isHidden) {
        extraArticles.forEach(el => el.classList.remove('hidden'));
        button.innerHTML = `
            Mostrar Menos
            <svg class="ml-2 -mr-0.5 h-4 w-4"
                 xmlns="http://www.w3.org/2000/svg" fill="none"
                 ...
                 stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round"
                      d="M8.25 15.75l7.5-7.5 7.5 7.5"></path>
            </svg>
        `;
    } else {
        extraArticles.forEach(el => el.classList.add('hidden'));
        button.innerHTML = `
            Mostrar Más
            <svg class="ml-2 -mr-0.5 h-4 w-4"
                 xmlns="http://www.w3.org/2000/svg" fill="none"
...
@endif
        ...
        <div>
            ...
            @if($openModalFilter)
                <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
                    <div
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" min="1890" max="{{now()->year}}" wire:model="from_date"
                                   class="w-full p-2 mt-1 border rounded-md">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">Hasta</label>
                            <input type="number" min="1890" max="{{now()->year}}" wire:model="to_date"
                                   class="w-full p-2 mt-1 border rounded-md">
                        </div>
                        @if($fontOptions[0] !== 'PubMed')
                            {{-- Seleccionar/Deseleccionar todos --}}
                            <div class="flex justify-between mb-2">
                                <button wire:click="selectAll" class="text-sm text-blue-600 hover:underline">Seleccionar
                                    todos
                                </button>
                                <button wire:click="deselectAll" class="text-sm text-red-600 hover:underline">
                                    Deseleccionar
                                    todos
                                </button>
                            </div>
                        ...
                        {{-- Seleccionar/Deseleccionar todos --}}
                        <div class="flex justify-between mb-2">
                            <button wire:click="selectTypeAll" class="text-sm text-blue-600 hover:underline">Seleccionar
                                todos
                            </button>
                            <button wire:click="deselectTypeAll" class="text-sm text-red-600 hover:underline">
                                Deseleccionar
                                todos
                            </button>
                        </div>
                ...
                @if(!Auth::user()->hasAnyRole('root') && Auth::user()->status == 0)
                    <div
                        class="absolute inset-0 bg-white/80 z-50 flex flex-col items-center justify-center rounded-2xl">
                        <button wire:click="closeFilters"
                                class="text-2xl  text-white absolute top-0 right-0 bg-[#195b81] w-10 h-10 rounded-xl">
                            &times;
                        </button>
                        <a
                            href="{{ route('planes') }}"
        ...
        startTyping(content) {
            ...
            this.interval = setInterval(() => {
                if (this.index < this.fullContent.length) {
                    ...
                    // Lanzar evento al finalizar el tipeo
                    window.dispatchEvent(new CustomEvent('typing-finished'));
                }
            }, 10);
        }
    };
