        ...
        <div class="  flex flex-col overflow-hidden gap-[12px] text-center mt-2 w-full">
            @foreach($chatHistory as $chat)
                <button wire:click="selectChat({{ $chat->id }})"
                        ...
                        {{ $activeChatId == $chat->id ?  : '' }} "><i class="mr-3 fa-regular fa-message"></i>
                    Chat #{{ $chat->id }} <br>
                    <span class="ml-3 text-xs">{{ $chat->created_at->format('d/m/Y') }}</span>
                </button>
            @endforeach
        </div>
    </div>
            ...
            <form wire:submit.prevent="sendMessage" class="relative flex flex-col gap-2">
                @if(($queryCount <= 99))
                    @if($config)
                        <div class="flex items-center gap-3 mb-3">
                            <div>
                                ...
                                <select wire:model.live="modeloIA" class="px-2 py-1 text-sm border rounded-md">
                    ...
                    <div class="mx-auto max-w- 67rem">
                        <div class="absolute left-0 max-w-xs p-3 mt-6 ml-6 bg-white rounded-lg shadow bocadillo bocadillo-left">
                        ...
                        <div class="absolute  right-0 mb-6 mr-6 max-w-xs p-3 bg-[#195b81] shadow rounded-lg text-white bocadillo bocadillo-right">
                            <p class="text-sm">
                                📌 El Modelo Advance es perfecto para consultas de investigacion mas detalladas,
